...
- `src/pages/api/create-checkout-session.js` creates the Checkout Session with `mode: "subscription"` for recurring plans. Only the **base service line item** (always index 0 in `calculatePrice()`'s `lineItems`, see `pricing.js`) gets `price_data.recurring` — every add-on/stepper line item after it is left as a plain one-time price on purpose. Checkout Sessions in subscription mode natively support mixing recurring + one-time line items in the same cart: Stripe bills the one-time ones once, on the first invoice only, then drops them automatically for every renewal after that. This matches how customers actually think about add-ons on a recurring plan ("clean my oven this time," not "charge me for that every week forever") — the alternative of making every add-on recur indefinitely was the original behavior and got changed after a live discussion flagged it as a surprise-bill risk. `PricingConfigurator.astro` shows a note next to the price for recurring plans with add-ons selected, making this clear before checkout. The customer/slot metadata is duplicated onto `subscription_data.metadata` (including `lastVisitStart`/`lastVisitEnd`, seeded to the first picked slot) — this is what lets the webhook figure out future visit dates, since Stripe doesn't automatically carry Checkout Session metadata onto the subscription or its invoices.
