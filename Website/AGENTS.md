...
- `src/pages/api/create-checkout-session.js` creates the Checkout Session with `mode: "subscription"` for recurring plans. Only the **base service line item** (always index 0 in `calculatePrice()`'s `lineItems`, see `pricing.js`) is included as a Checkout line item with `price_data.recurring` set — add-on/stepper line items are deliberately left out of `line_items` entirely for recurring bookings. This matches how customers actually think about add-ons on a recurring plan ("clean my oven this time," not "charge me for that every week forever") — the alternative of making every add-on recur indefinitely was the original behavior and got changed after a live discussion flagged it as a surprise-bill risk. `PricingConfigurator.astro` shows a note next to the price (plus one in the add-ons section itself) for recurring plans with add-ons selected, making this clear before checkout.
  - **Why add-ons aren't just one-time Checkout line items instead:** that was the first approach tried, and Stripe's API rejects it outright — `"You cannot set 'proration_behavior' to 'none' in a Checkout Session with one-time prices."` Since `proration_behavior: "none"` is exactly the mechanism this app uses to defer the first charge to the actual cleaning date (see "Billing date alignment" below), that combination is a hard no. Confirmed via a live test that failed in production before landing on the current approach.
  - **What actually happens instead:** add-on line items (label + amount) get JSON-encoded into a short-lived `addonLines` key on the Checkout Session's own metadata (not the subscription's — it's only needed once, and keeping it off subscription metadata avoids bumping into Stripe's per-value size limit). In `stripe-webhook.js`'s `checkout.session.completed` handler, once the subscription exists, each add-on becomes a pending invoice item via `stripe.invoiceItems.create({ customer, subscription, amount, description })` with no `invoice` specified — Stripe automatically sweeps items like that into whatever that subscription's *next* invoice turns out to be. Since `proration_behavior: "none"` means the next invoice is the deferred first-visit invoice, the add-ons land on the same charge as the first cleaning, then never appear again since they're one-off items, not a recurring price.
