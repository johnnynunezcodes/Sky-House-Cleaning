...
- `src/pages/api/create-checkout-session.js` creates the Checkout Session with `mode: "subscription"` for recurring plans. All line items (base service **and** add-ons) are included as real recurring Checkout line items, so Stripe's own payment page shows the customer their true first-charge total — but add-ons still shouldn't keep billing every cycle forever. This matches how customers actually think about add-ons on a recurring plan ("clean my oven this time," not "charge me for that every week forever") — the alternative of making every add-on recur indefinitely was the original behavior and got changed after a live discussion flagged it as a surprise-bill risk. `PricingConfigurator.astro` shows a note next to the price (plus one in the add-ons section itself) for recurring plans with add-ons selected, making this clear before checkout.
  ...
  - **Cosmetic gotcha, found via live testing:** because every line item (base + add-ons) is marked `recurring`, Stripe's own Checkout page prints a per-line caption like "Billed weekly — $20.00/week after" under each add-on too — which reads as if the add-on itself recurs forever, even though it's actually removed after the first invoice (see above). There's no per-line-item way to override that caption. Fixed with `custom_text.submit.message` in `create-checkout-session.js` (a documented, stable Checkout Session field) — a plain-language note shown directly above the Subscribe button, added only when `isRecurring` and there's at least one add-on line, stating the add-on(s) are for the first cleaning only and naming the real ongoing per-cycle price (base price alone). Kept deliberately short and plain per `Branding/Verbal Branding.md`.
